...
This is optional, but you can add more Panels if you like, for example: The JVM RSS Value `process_resident_memory_bytes` (set the Visualization to `Gauge` and the Unit in _Field_ tab to `bytes(IEC)` on the _Visualization_ tab, and the title to `Memory` on the _Panel_ tab). It could look like:
