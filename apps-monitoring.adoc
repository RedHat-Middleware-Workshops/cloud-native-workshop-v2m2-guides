...
In this step, we will add the Quarkus extensions to the Inventory application for using *smallrye-opentracing*. Run the following commands to add the tracing extension via VS Code Terminal:

[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="smallrye-opentracing" -f $PROJECT_SOURCE/cloud-native-workshop-v2m2-labs/inventory
----
...
In the `inventory` project under `cloud-native-workshop-v2m2-labs` workspace, open `src/main/resources/application.properties` file and add the following configuration via VS Code Terminal:
...
mvn package -DskipTests -f $PROJECT_SOURCE/cloud-native-workshop-v2m2-labs/inventory
----
...
[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="micrometer-registry-prometheus" -f $PROJECT_SOURCE/cloud-native-workshop-v2m2-labs/inventory
----
...
----
oc project {{ USER_ID }}-inventory && \
mvn clean package -DskipTests -f $PROJECT_SOURCE/cloud-native-workshop-v2m2-labs/inventory
----
