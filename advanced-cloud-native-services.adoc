...
[INFO]
====
It sometimes takes time to create a new build image in OpenShift that depends on network latency. So if you got failed to deploy catalog-service with an error, *Error from server (NotFound): services "catalog-springboot" not found*, please try again with delay via the following command:
====
